...
export const DashboardChart = ({ type = 'line', title = 'Untitled', data = [], loading = false }) => {
  if (loading) {
    return (
      <Card className="bg-white rounded-xl border border-slate-200 shadow-sm">
        <div className="px-6 py-4 border-b border-slate-100">
    ...
    datasets: [
      {
        ...
        fill: type === 'line',
        backgroundColor: type === 'doughnut' ? ['#6366F1', '#8B5CF6', '#EC4899', '#F43F5E'] : '#6366F1',
        borderColor: '#6366F1',
        tension: 0.4
      }
    ]
  ...
  const renderChart = () => {
    switch (type) {
      ...
      case 'doughnut': return <Doughnut data={chartData} options={{ ...chartOptions, scales: undefined }} />;
      case 'line':
