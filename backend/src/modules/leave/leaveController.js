...
const reviewLeave = asyncHandler(async (req, res) => {
  ...
  if (result.success) {
    ...
    sendNotification(leaveRequest.employeeId, {
      ...
      message: `Your leave request from ${leaveRequest.startDate} to ${leaveRequest.endDate} has been ${leaveRequest.status.toLowerCase()}.`,
      leaveId: leaveRequest.id,
      startDate: leaveRequest.startDate,
      endDate: leaveRequest.endDate,
      status: leaveRequest.status,
    ...
    if (leaveRequest.status === 'Approved') {
      ...
      sendNotification(leaveRequest.employeeId, {
        type: "LEAVE_BALANCE_UPDATED",
        title: "Leave Balance Updated",
        message: `Remaining leave balance: ${balance.remaining} days.`,
        balance
      });
...
const cancelLeave = asyncHandler(async (req, res) => {
  ...
  if (result.success) {
    ...
    sendNotification(req.user.id, {
      ...
      message: `Leave request from ${leaveRequest.startDate} to ${leaveRequest.endDate} cancelled.`,
      leaveId: leaveRequest.id,
      startDate: leaveRequest.startDate,
      endDate: leaveRequest.endDate,
      days: leaveRequest.daysRequested
    ...
    if (leaveRequest.managerId) {
      sendNotification(leaveRequest.managerId, {
        type: "LEAVE_CANCELLED_MANAGER",
        title: "Leave Cancelled",
        message: `${req.user.name || 'Employee'} cancelled leave request.`,
        leaveId: leaveRequest.id,
        employeeId: req.user.id,
        employeeName: req.user.name
      });
...
const resetLeaveBalances = asyncHandler(async (req, res) => {
  const result = await leaveService.yearlyLeaveReset(req.body.year);
  ...
  for (const employeeId of allEmployees) {
    sendNotification(employeeId, {
      ...
      message: `Leave balance reset to ${req.body.totalAnnual} days.`,
      totalAnnual: req.body.totalAnnual,
      year: req.body.year
    });
  }
...
const getLeaveById = asyncHandler(async (req, res) => {
  const result = await leaveService.findLeaveRequestById(Number(req.params.id));
  res.status(200).json({
    success: true,
    data: result
  });
...
const listTeamLeaves = asyncHandler(async (req, res) => {
  const result = await leaveService.listTeamLeaves(req.user.id);
  res.status(200).json(result);
});
