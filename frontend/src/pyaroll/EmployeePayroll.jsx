...
// src/components/EmployeePayroll.jsx
import React, { useState, useMemo, useEffect } from 'react';
import { useQuery } from '@tanstack/react-query';
import toast, { Toaster } from 'react-hot-toast';
import { useAuth } from '../context/AuthContext';
...
const ErrorState = ({ error, onRetry }) => (
  <div className="text-center py-10">
    <p className="text-red-500 mb-4">{error}</p>
    <button onClick={onRetry} className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors" aria-label="Retry loading payroll data">Retry</button>
  </div>
);
...
const Pagination = ({ currentPage, totalPages, onPageChange }) => {
  ...
  return (
    <div className="px-6 py-3 flex items-center justify-between border-t border-gray-200 bg-gray-50">
      <button onClick={() => onPageChange(currentPage - 1)} disabled={currentPage === 1} className="px-3 py-1 border rounded disabled:opacity-50" aria-label="Previous page">Previous</button>
      <span className="text-sm text-gray-700">Page {currentPage} of {totalPages}</span>
      <button onClick={() => onPageChange(currentPage + 1)} disabled={currentPage === totalPages} className="px-3 py-1 border rounded disabled:opacity-50" aria-label="Next page">Next</button>
    </div>
  );
};
...
const EmployeePayroll = () => {
  ...
  const trendData = useMemo(() => {
    return [...allHistory]
      .map((item) => ({ month: `${item.month}/${item.year}`, net: item.netSalary || 0 }))
      .reverse();
  }, [allHistory]);
  ...
  useEffect(() => { setCurrentPage(1); }, [allHistory.length]);
                    ...
                    {paginatedHistory.map((record) => {
                      ...
                      return (
                        <tr key={record.id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">₹{gross.toLocaleString('en-IN')}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">₹{deductions.toLocaleString('en-IN')}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">₹{net.toLocaleString('en-IN')}</td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${statusStyle}`}>{record.status}</span>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm">
                            <button onClick={() => openPayslip(record)} className="text-blue-600 hover:text-blue-900 font-medium" aria-label={`View payslip for ${record.month}/${record.year}`}>View Payslip</button>
                          </td>
                        </tr>
                      );
                    })}
