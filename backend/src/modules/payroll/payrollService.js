...
//           ctcMonthly: monthlySalary,
//           ctcAnnual: monthlySalary * 12,
//           bonus: 0,
//           otMinutes: 0,
//           isMetro: true,
//         });
...
//     // ── Cache bust (FIXED month/year key)
//     await Promise.all(
//       results.map(({ employeeId }) =>
//         clearCacheKeys([
//           `dashboard_summary:${employeeId}:${value.year}:${value.month}`,
//         ]).catch((err) =>
//           logger.error({
//             event: 'CACHE_BUST_FAILED',
//             employeeId,
//             error: err.message,
//           })
//         )
...
//     // ── Notifications (kept but safer)
//     for (const r of results) {
//       sendNotification(r.employeeId, {
//         type: 'PAYROLL_PROCESSED',
//         title: 'Salary Processed',
//         message: `Your salary for ${value.month}/${value.year} has been processed.`,
...
//     eventBus.emit('PAYROLL_PROCESSED', {
//       month: value.month,
//       year: value.year,
//       results,
//       totalAmount,
...
// // ─────────────────────────────────────────────────────────────
// // PAYSLIP DOWNLOAD (FIXED FIELD MISMATCH)
// // ─────────────────────────────────────────────────────────────
...
const processPayroll = async ({
  ...
  try {
    ...
    await Promise.all(
      results.map(({ employeeId }) =>
        clearCacheKeys([
          `dashboard_summary:${employeeId}:${year}:${month}`,
        ]).catch((err) =>
          logger.error({
            event: 'CACHE_BUST_FAILED',
            employeeId,
