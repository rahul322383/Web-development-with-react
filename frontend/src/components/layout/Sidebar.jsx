...
import {
  LayoutDashboard, Users, Calendar, Receipt, Bell, LogOut, Home, Info,
  Sparkles, CreditCard, PlayCircle, BookOpen, Briefcase, Shield, Scale,
  Lock, HelpCircle, LogIn, UserPlus, Settings, User, ClipboardList,
  BarChart, MessageSquare, ChevronLeft, ChevronRight, Clock, Building2,
  FileText, X, IndianRupee, BarChart3,
} from 'lucide-react';
...
const BASE_NAVIGATION = [
  { name: 'Dashboard', href: '/dashboard', icon: LayoutDashboard, roles: Object.values(ROLES), showForAll: true },
  ...
  { name: 'Profile', href: '/profile', icon: User, roles: Object.values(ROLES), showForAll: true },
  { name: 'Leave Management', href: '/leave', icon: Calendar, roles: Object.values(ROLES) },
  { name: 'Attendance', href: '/attendance', icon: Calendar, roles: Object.values(ROLES) },
  { name: 'Shift Management', href: '/shift-management', icon: Clock, roles: Object.values(ROLES) },
  ...
  { name: 'Company', href: '/company', icon: Building2, roles: Object.values(ROLES) },
  { name: 'Expenses', href: '/expenses', icon: Receipt, roles: Object.values(ROLES) },
  ...
  { name: 'Notifications', href: '/notifications', icon: Bell, roles: Object.values(ROLES) },
  { name: 'Settings', href: '/settings', icon: Settings, roles: Object.values(ROLES) },
];
...
export const Sidebar = ({ isOpen, onClose, pendingCount = 0, onCollapseChange }) => {
  ...
  // On mobile the sidebar is never "collapsed" — it's either fully open or hidden
  ...
  // ── Sidebar width ─────────────────────────────────────────────────────────
  // We must use full class strings — Tailwind JIT cannot evaluate template expressions.
  const desktopWidthClass = effectiveCollapsed ? 'lg:w-20' : 'lg:w-72';
    ...
    <>
      ...
      <aside
        className={`
          ...
          w-[85%] max-w-xs ${desktopWidthClass}
