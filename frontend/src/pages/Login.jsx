...
//   // ✅ Fixed redirect logic - only redirect after auth check is complete
//   useEffect(() => {
...
//   if (isLoading) {
//     return (
//       <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 via-white to-purple-50 dark:from-slate-950 dark:via-slate-950 dark:to-slate-950">
...
//       if (!result.success) {
//         const errorMessage = result.error?.response?.data?.message || 'Login failed. Please check your credentials.';
...
const Input = React.memo(({
  label, type = 'text', icon: Icon, error, touched,
  showPasswordToggle, onTogglePassword, showPassword, ...props
}) => {
  const [isFocused, setIsFocused] = useState(false);
...
const Login = () => {
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      // Navigation handled by useEffect when isAuthenticated flips
    } catch {
      toast.error('An unexpected error occurred. Please try again.');
    } finally {
      setLoading(false);
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 relative overflow-hidden flex items-center justify-center px-4">
      ...
      >
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xl shadow-slate-200/60 dark:shadow-slate-950/60 overflow-hidden">
          ...
          <div className="px-8 pt-7 pb-5 border-b border-slate-100 dark:border-slate-800 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm">Sign in to your HRMS account</p>
            </div>
          </div>

          {/* Form */}
          <form onSubmit={handleSubmit} noValidate className="p-8 space-y-5">
            <Input label="Email Address" type="email" icon={Mail} required {...field('email')} />
            <Input
              label="Password" type="password" icon={Lock} required
              showPasswordToggle showPassword={showPassword} onTogglePassword={togglePassword}
              {...field('password')}
            />
        ...
        >
          ...
          <p className="text-xs text-slate-400">© {new Date().getFullYear()} HRMS. All rights reserved.</p>
        </motion.div>
      </motion.div>
    </div>
  );
