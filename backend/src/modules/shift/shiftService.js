...
//     const id = resolveCompanyId(actor);
//     if (!id) {
//         return fail(
...
//     if (typeof companyIdOrFail === 'object') return companyIdOrFail; // fail object
...
//         await logAuditEvent({
//             userId: actor.id, moduleName: 'Shift', actionType: 'CREATE',
//             oldData: null, newData: shift, ipAddress: actor.ip,
//         });
...
//         await logAuditEvent({
//             userId: actor.id, moduleName: 'Shift', actionType: 'UPDATE',
//             oldData: old, newData: shift, ipAddress: actor.ip,
//         });
...
//         const active = await ShiftAssignment.count({
//             where: { shiftId, companyId, isActive: true },
//         });
//         if (active > 0) {
//             return fail('Cannot delete shift with active employee assignments. Reassign employees first.', 409);
//         }
...
//     if (typeof companyIdOrFail === 'object') {
//         return companyIdOrFail;
//     }
...
//     } catch (err) {
//         logger.error({ event: 'GET_EMPLOYEE_SHIFT_FAILED', employeeId, error: err.message });
//         return DEFAULT_SHIFT;  // safe fallback — never crash a check-in/out
//     }
// };
...
//                 { model: User, as: 'assignor', attributes: ['id', 'firstName', 'lastName'] },
//             ],
//             order: [['effectiveFrom', 'DESC']],
//         });
...
//     // Any negative value means midnight was crossed (night shift)
...
const checkPerm = (actor, perm) => {
    ...
    if (!result.allowed) {
        return fail(result.message || 'Forbidden', 403);
    }
...
const createShift = async ({ actor, data }) => {
    ...
    try {
        // IMPORTANT:
        // Your DB column = company_id
        // Model field = companyId (with field mapping)
...
const deleteShift = async ({ actor, shiftId }) => {
    ...
    try {
        ...
        // IMPORTANT FIX:
        // ShiftAssignment model DOES NOT have companyId field
        // so remove companyId from where clause
...
const getEmployeeShift = async (employeeId, date) => {
    try {
        ...
        return {
            ...assignment.shift.toJSON(),
            isDefault: false,
        };
    } catch (err) {
        logger.error({
            event: 'GET_EMPLOYEE_SHIFT_FAILED',
            employeeId,
            error: err.message,
        });
...
const getEmployeeShiftHistory = async ({
    ...
    try {
        ...
        logger.error({
            event: 'GET_SHIFT_HISTORY_FAILED',
            error: err.message,
        });
...
const calcMetricsWithShift = (
    ...
    const shiftStartM = toMins(
        shift.startTime ?? '09:00'
    );

    const shiftEndM = toMins(
        shift.endTime ?? '18:00'
    );
    ...
    const otAfter =
        shift.overtimeAfterMins ?? 0;

    const otRate =
        shift.overtimeRateMultiplier ?? 1.5;

    const standardM =
        shiftEndM - shiftStartM;
    ...
    const overtimeMinutes = Math.max(
        0,
        workedMinutes - standardM - otAfter
    );
