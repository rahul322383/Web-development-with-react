...
const getHistory = (userId) => {
    return conversationStore.get(String(userId)) || [];
};
...
const clearHistoryStore = (userId) => {
    conversationStore.delete(String(userId));
};
...
const enrichUser = async (user) => {
    try {
        ...
        user.designation = fullUser.designation || null;
        user.name = `${fullUser.firstName} ${fullUser.lastName}`;

        return user;
    } catch (error) {
        logError('ENRICH_USER_ERROR', error, {
            userId: user?.id,
        });
...
const handleChat = async (req, res) => {
    try {
        ...
        // SAVE USER MESSAGE
        // ─────────────────────────────────────────────────
        appendHistory(user.id, 'user', cleanMessage);
...
const searchPolicy = async (req, res) => {
    try {
        ...
        return res.status(200).json({
            success: true,
            message:
                'Use POST /ai/chat with: "search policy for leave policy"',
        });
    } catch (error) {
        logError('AI_POLICY_SEARCH_ERROR', error);
