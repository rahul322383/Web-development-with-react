...
// ─── Button (was missing, used in Pagination) ───────────────────────────────
const Button = ({
  children,
  onClick,
  disabled = false,
...
// ─── StatCard ────────────────────────────────────────────────────────────────
// color now accepts BOTH semantic keys ("total","approved",...) AND Tailwind
// gradient keys ("indigo","emerald",...) so existing callers keep working.
const StatCard = ({
  title,
  value,
  icon: Icon,
  ...
  const gradients = {
    ...
    blue: 'from-blue-500 to-blue-600',
    purple: 'from-purple-500 to-purple-600',
    // Semantic aliases used in DepartmentDashboard
    ...
    days: 'from-purple-500 to-indigo-500'
  };
...
const EmptyState = ({
  ...
  >
    ...
    <div className="relative max-w-md mx-auto">
      ...
      <p className="mt-2 text-gray-600 dark:text-gray-400">
        {description || (hasFilters ? 'Try adjusting your filters to see more results.' : "There's nothing here yet.")}
      </p>
      <div className="mt-6 flex justify-center gap-3 flex-wrap">
        {action}
        {!action && onRefresh && (
          <button
            onClick={onRefresh}
            className="inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg text-white bg-indigo-600 hover:bg-indigo-700 transition-all hover:shadow-lg"
          >
...
const ErrorState = ({
  ...
  const displayMessage = message || error || 'An unexpected error occurred. Please try again.';
  return (
    <motion.div
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      ...
      <div className="relative max-w-md mx-auto">
        ...
        <div className="mt-6 flex justify-center gap-3 flex-wrap">
          ...
          {!action && onRetry && (
            <button
              onClick={onRetry}
              className="inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg text-white bg-rose-600 hover:bg-rose-700 transition-all hover:shadow-lg"
            >
              <RefreshCw className="w-4 h-4 mr-2" />
              Try Again
            </button>
...
const RoleBadge = ({ role }) => {
  const styles = {
    ...
    manager: 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300 border-blue-200 dark:border-blue-700',
    employee: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 border-green-200 dark:border-green-700',
    hr: 'bg-pink-100 text-pink-800 dark:bg-pink-900/30 dark:text-pink-300 border-pink-200 dark:border-pink-700'
  };
  const key = role?.toLowerCase() || 'employee';
  return (
    <span className={`px-2.5 py-1 text-xs font-medium rounded-full border ${styles[key] || styles.employee}`}>
      {role ? role.charAt(0).toUpperCase() + role.slice(1) : 'Employee'}
    </span>
  );
};
...
const StatusBadge = ({ isActive }) => (
  <span
    className={`inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium rounded-full border ${isActive
        ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 border-green-200 dark:border-green-700'
        : 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-600'
      }`}
  >
...
const UserCard = ({ user, isCurrentUser }) => (
  <motion.div
    initial={{ opacity: 0, scale: 0.95 }}
    animate={{ opacity: 1, scale: 1 }}
    className={`bg-white dark:bg-gray-800 rounded-lg p-4 border ${isCurrentUser
        ? 'border-blue-500 dark:border-blue-400 ring-2 ring-blue-200 dark:ring-blue-900'
        : 'border-gray-200 dark:border-gray-700'
      }`}
  >
    ...
    <div className="space-y-2 text-sm">
      ...
      <div className="flex items-center gap-2 text-gray-600 dark:text-gray-400">
        <span className="font-medium">₹{Number(user.baseSalary).toLocaleString('en-IN')}</span>
      </div>
      <div className="flex items-center justify-between">
        <StatusBadge isActive={user.isActive} />
        {user.employeeCode && (
          <span className="text-xs text-gray-400 font-mono">{user.employeeCode}</span>
        )}
      </div>
    </div>
  </motion.div>
    ...
    <table className="w-full">
      <thead>
        <tr className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
          ...
          <th className="text-left py-3 px-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Code</th>
          <th className="text-left py-3 px-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Role</th>
          <th className="text-left py-3 px-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Department</th>
          <th className="text-left py-3 px-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Salary</th>
        ...
        <AnimatePresence>
          {users.map((user) => (
            <motion.tr
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className={`border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors ${user.id === currentUserId ? 'bg-blue-50 dark:bg-blue-900/20' : ''
                }`}
            >
              <td className="py-3 px-4">
                <div className="flex items-center gap-3">
              ...
              <td className="py-3 px-4 text-sm font-mono text-gray-500 dark:text-gray-400">
                {user.employeeCode || '—'}
              </td>
              <td className="py-3 px-4">
                <RoleBadge role={user.role} />
              ...
              <td className="py-3 px-4 text-gray-700 dark:text-gray-300">{user.department}</td>
              <td className="py-3 px-4 text-gray-700 dark:text-gray-300">
                ₹{Number(user.baseSalary).toLocaleString('en-IN')}
              </td>
              <td className="py-3 px-4">
                <StatusBadge isActive={user.isActive} />
              </td>
...
// ─── getStatusBadgeStyle ──────────────────────────────────────────────────────
const getStatusBadgeStyle = (status = '') => {
  const base = 'inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border transition-colors duration-200';
  const map = {
    pending: 'bg-yellow-50 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-200 border-yellow-200 dark:border-yellow-700',
    approved: 'bg-green-50 dark:bg-green-900/30 text-green-800 dark:text-green-200 border-green-200 dark:border-green-700',
    rejected: 'bg-red-50 dark:bg-red-900/30 text-red-800 dark:text-red-200 border-red-200 dark:border-red-700',
    cancelled: 'bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200 border-gray-200 dark:border-gray-700'
  };
  return `${base} ${map[status.toLowerCase()] || 'bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200 border-gray-200 dark:border-gray-700'}`;
};
...
const AlertMessage = ({ type = 'success', message, onClose, onRetry }) => {
  ...
  const styles = {
    container: isSuccess
      ? 'bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 border-green-200 dark:border-green-800'
      : 'bg-gradient-to-r from-red-50 to-rose-50 dark:from-red-900/20 dark:to-rose-900/20 border-red-200 dark:border-red-800',
    text: isSuccess ? 'text-green-800 dark:text-green-200' : 'text-red-800 dark:text-red-200',
    icon: isSuccess ? 'text-green-500 dark:text-green-400' : 'text-red-500 dark:text-red-400',
    closeBtn: isSuccess ? 'text-green-500 hover:text-green-700 dark:hover:text-green-300' : 'text-red-500 hover:text-red-700 dark:hover:text-red-300'
  };
  return (
    <div className="mb-6">
      <div className={`border rounded-xl p-4 shadow-sm ${styles.container}`}>
        <div className="flex items-start gap-3">
          <div className="flex-shrink-0">
            {isSuccess ? (
              <svg className={`w-5 h-5 ${styles.icon}`} fill="currentColor" viewBox="0 0 20 20">
          ...
          <div className="flex-1">
            ...
            {type === 'error' && onRetry && (
              <button onClick={onRetry} className="mt-2 text-sm font-medium text-red-600 dark:text-red-400 hover:underline">
                Try Again
              </button>
            )}
          </div>
          <button onClick={onClose} className={`${styles.closeBtn} transition-colors`}>
            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
            </svg>
          </button>
...
// ─── getStatusIcon ────────────────────────────────────────────────────────────
const getStatusIcon = (status) => {
  const cls = 'w-5 h-5 transition-colors duration-200';
  switch (status?.toLowerCase()) {
    case 'approved':
      return (
        <svg className={`${cls} text-green-600 dark:text-green-400`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
      );
    case 'rejected':
      return (
        <svg className={`${cls} text-red-600 dark:text-red-400`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
      );
    default:
...
// ─── LoadingSpinner ───────────────────────────────────────────────────────────
const LoadingSpinner = ({ size = 'default', text = 'Loading...' }) => {
  const sizeClasses = { small: 'h-8 w-8', default: 'h-12 w-12', large: 'h-16 w-16' };
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="text-center">
        <div className={`animate-spin rounded-full ${sizeClasses[size]} border-b-2 border-blue-600 dark:border-blue-400 mx-auto`} />
        <p className="mt-4 text-gray-600 dark:text-gray-400">{text}</p>
      </div>
    </div>
  );
...
const ProgressBar = ({ value, max = 100, color = 'indigo', showPercentage = true, height = 'h-2' }) => {
  ...
  return (
    <div className="space-y-1.5">
      <div className={`w-full bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden ${height}`}>
        <div
          className={`${colors[color] || colors.indigo} ${height} rounded-full transition-all duration-500 ease-out`}
          style={{ width: `${percentage}%` }}
        />
      </div>
      {showPercentage && (
      ...
      <div className="flex items-center gap-2">
        ...
        {pageNumbers.map((n) => (
          <Button
            ...
            className={`w-8 h-8 ${currentPage === n ? 'bg-indigo-600 dark:bg-indigo-500 text-white' : ''}`}
          >
            {n}
          </Button>
        ))}
