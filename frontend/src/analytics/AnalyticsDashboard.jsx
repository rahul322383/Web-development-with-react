...
// // ─── filter bar ────────────────────────────────────────────────────
// const FilterBar = ({ filters, onChange, departmentDisabled }) => (
...
//         ...inputStyle,
//         minWidth: 200,
//         background: departmentDisabled ? '#F3F4F6' : '#fff',
//         cursor: departmentDisabled ? 'not-allowed' : 'text',
//       }}
//     />
//     <button
...
//   const isDepartmentLocked = !!user && !isPrivileged;

//   const lockedDepartment =
//     isDepartmentLocked ? user?.department || '' : '';
...
//   // ✅ Sync department after auth loads
//   useEffect(() => {
//     if (lockedDepartment) {
//       setFilters(prev => ({
//         ...prev,
...
//   const finalDepartment = isDepartmentLocked ? lockedDepartment : filters.department;
...
//     deptPerformance: safeArray(data?.departmentPerformance),
...
//             {' '}({user.primaryRole})
//             {isDepartmentLocked && ` — Restricted to ${lockedDepartment}`}
//           </p>
//         )}
//       </div>
...
//           }
//         }}
//         departmentDisabled={isDepartmentLocked}
//       />
...
const FilterBar = ({ filters, onChange, departmentValue, departmentDisabled, lockedDepartment }) => (
  <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap' }}>
    <input
      type="date"
      value={filters.startDate}
    ...
    <input
      ...
      style={{
        ...
        background: departmentDisabled ? '#F3F4F6' : '#fff',
        cursor: departmentDisabled ? 'not-allowed' : 'text',
        color: departmentDisabled ? COLORS.muted : '#111827',
      }}
    />
    <button
      onClick={() => onChange({
        startDate: '',
        endDate: '',
        department: departmentDisabled ? lockedDepartment : '',
      })}
      style={{ ...inputStyle, background: COLORS.bg, cursor: 'pointer', color: COLORS.muted }}
    >
      Reset
...
const EmptyState = ({ message }) => (
  <p style={{ color: COLORS.muted, fontSize: 13 }}>{message}</p>
);
...
export default function AnalyticsDashboard() {
  ...
  const isDepartmentLocked =
    user && user.role !== ROLES.ADMIN && user.role !== ROLES.HR;
  const lockedDepartment = isDepartmentLocked ? user.department : '';
  ...
  const totalLeaveDays = useMemo(
    () => leaveMonthly.reduce((s, r) => s + safeNumber(r?.totalDays), 0),
    [leaveMonthly]
  );

  const totalLeaveRequests = useMemo(
    () => leaveMonthly.reduce((s, r) => s + safeNumber(r?.leaveCount), 0),
    [leaveMonthly]
  );

  const activeEmployees = useMemo(
    () => attritionOverall.employeesAtEnd ?? deptPerformance.reduce((s, d) => s + safeNumber(d?.headCount), 0),
    [attritionOverall, deptPerformance]
  );

  const totalLeaveStatus = useMemo(
    () => leaveStatusBreakdown.reduce((s, r) => s + safeNumber(r?.count), 0) || 1,
    [leaveStatusBreakdown]
  );
  ...
  return (
    <div style={{ padding: '24px 32px', background: COLORS.bg, minHeight: '100vh' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        {user && (
          <p style={{ fontSize: 13, color: COLORS.muted, marginTop: 6 }}>
            👤 {user.name} ({user.role}){isDepartmentLocked && ` — View limited to ${lockedDepartment}`}
          </p>
        )}
        <p style={{ fontSize: 13, color: COLORS.muted, marginTop: 4 }}>
          {periodText || 'Loading period…'}
      ...
      <FilterBar
        ...
        onChange={(newFilters) => {
          if (isDepartmentLocked) {
            setFilters({ ...newFilters, department: lockedDepartment });
          } else {
            setFilters(newFilters);
          }
        }}
      ...
      <div style={{
        ...
        <Card title="Attrition by Department">
          {isLoading ? <Skeleton /> : attritionByDept.length === 0 ? (
      ...
      <div style={{
        ...
        <Card title="Department Performance">
          {isLoading ? <Skeleton /> : deptPerformance.length === 0 ? (
        ...
        <Card title="Leave Status Breakdown">
          {isLoading ? <Skeleton h={240} /> : leaveStatusBreakdown.length === 0 ? (
            <EmptyState message="No leave data available." />
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {leaveStatusBreakdown.map((item, i) => {
                const pct = ((safeNumber(item?.count) / totalLeaveStatus) * 100).toFixed(1);
                const color = statusColors[item?.status] ?? COLORS.muted;
      ...
      <Card title="Cost per Employee by Department">
        {isLoading ? <Skeleton h={220} /> : costByDept.length === 0 ? (
