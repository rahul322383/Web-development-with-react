...
const deleteUser = async (id, actor, ipAddress) => {
  ...
  try {
    const existing = await userRepository.findUserById(id);
    if (!existing) return fail('User not found', 404);
    if (!existing.isActive) return fail('User is already deactivated', 409);
...
// ─────────────────────────────────────────────────────────────
// ASSIGN MANAGER
// ✅ FIXED: now uses userRepository instead of bare User.findByPk
//    which was never imported — caused runtime crash on every call
// ─────────────────────────────────────────────────────────────
