...
const PendingLeaves = () => {
  ...
  const fetchLeaves = useCallback(async () => {
    ...
    try {
      ...
      if (response?.success) {
        ...
        leaves = response?.data?.data || response?.data || [];
      } else if (Array.isArray(response)) {
        leaves = response;
      }
  ...
  // Handle leave approval/rejection
  const handleReview = async (leaveId, action, decisionNote = '') => {
    // Confirmation to prevent misclicks
    if (!window.confirm(`Are you sure you want to ${action.toLowerCase()} this leave?`)) {
      return;
    }
    ...
    try {
      const response = await leaveApi.reviewLeave(leaveId, {
        ...
        requestId: Number(leaveId),
        decisionNote:
          decisionNote || `${action} by ${user?.primaryRole || meta?.role || 'manager'}`,
      });
      ...
      if (response?.success) {
        ...
        // Refetch all leaves to reflect correct status and move item to reviewed section
        await fetchLeaves();
      } else {
        throw new Error(response?.message || `Failed to ${action.toLowerCase()} leave`);
      }
